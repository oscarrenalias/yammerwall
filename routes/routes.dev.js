...
module.exports = function(config, io) {
  ...
  app.get("/dev/sendyam", function(req, res) {
    sendYam(generateTestYamData());
    res.render("dev/index.jade", {title:"Dev Support", result:"Yam successfully sent", data:{}})
  });

  app.post("/dev", function(req, res) {
    var testYam = generateTestYamData();
    ...
    _(amount).times(function() {      
      testYam.yam.body = {        
        rich: body, parsed: body, plain: body
      }

      sendYam(testYam);
    });

    res.render("dev/index.jade", {title:"Send test yam", result:"Yam successfully sent", data:req.query});
  });
  ...
  app.get("/dev/sendyam/attachment", function(req, res) {
    ...
    res.render("dev/index.jade", {title:"Dev Support", result:"Yam successfully send", data:{}})
  });  
}